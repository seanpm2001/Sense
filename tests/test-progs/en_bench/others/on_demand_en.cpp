...
void en_handler(en_frame* ef)
{
    ...
    printf("user en_handler: killing current process and starting ./safer_app_en\n");
    for (int i = 0; i < 100000; i++);
    asm volatile ("mfence");
    execve("./safer_app_en", NULL, NULL);
    ...
    __asm__("pop %rcx; pop %rcx; pop %rcx; pop %rdx; pop %rsi; pop %rdi; pop %r8; pop %r9; pop %r10; pop %r11; pop %r12; pop %r13; pop %r14; pop %r15; pop %rax; pop %rbp; ret");
    // __asm__("ret");
    // exit(0);
}
...
int main(int argc, char** argv) {
    ...
    printf("user application starts using OpenSSL.\n");
    int a = 0;
    ENBEGIN(en_handler, 0);
    ENPREFETCH((uint64_t)&a);
    for (volatile register int i = 0; i < 1; ++i) {
        // maccess(&a);
        // size_t time = rdtsc();
        // a = 0;
    ...
    printf("user application using OpenSSL ends.\n");
